...
public class Grid : MonoBehaviour
{
    ...
    {
        vGridWorldSize = new Vector2(p_Collider.bounds.size.x, p_Collider.bounds.size.z);
    ...
    public List<Node> GetNeighboringNodes(Node a_NeighborNode)
    {
        ...
        //Check the Upper Left side of the current node.
        icheckX = a_NeighborNode.iGridX - 1;
        icheckY = a_NeighborNode.iGridY + 1;
        if (icheckX >= 0 && icheckX < iGridSizeX)//If the XPosition is in range of the array
        {
        ...
        //Check the Upper Right side of the current node.
        icheckX = a_NeighborNode.iGridX + 1;
        icheckY = a_NeighborNode.iGridY + 1;
        if (icheckX >= 0 && icheckX < iGridSizeX)//If the XPosition is in range of the array
        {
        ...
        if (NodeArray != null)//If the grid is not empty
        {
            foreach (Node n in NodeArray)//Loop through every node in the grid
            {
                if (FinalPath != null)//If the final path is not empty
                {
                    ...
                    {
                        LineRenderer line = new GameObject("line" + counter).AddComponent<LineRenderer>();
                        line.material = lineMaterial;
                        line.positionCount = 2;
                        line.startWidth = 10f;
                        line.endWidth = 10f;
                        line.useWorldSpace = true;
                        line.SetPosition(0, n.vPosition - (Vector3.one * (fNodeDiameter - fDistanceBetweenNodes) * .5f));
                        line.SetPosition(1, n.vPosition + (Vector3.one * (fNodeDiameter - fDistanceBetweenNodes) * .5f));
                        counter++;
                        lines.Add(line);
                    }
                }
