...
public class Pathfinding : MonoBehaviour
{
    ...
    {
        GameObject[] names = new GameObject[2];
        int counter = 0;
        foreach (TMP_Dropdown d in d_List)
        {
            names[counter] = o_List[d.value];
            counter++;
        }
        StartPosition = names[0].transform;
        TargetPosition = names[1].transform;

    }

    private void FindPath()
    {
        a_StartPos = StartPosition.position;
        a_TargetPos = TargetPosition.position;
